...
{
    public partial class Form_ClusterKmean : Form, Form_Automatable
    {
        Process process = null;
        Model_Automator automationModel = null;

        public Form_ClusterKmean()
        {
            InitializeComponent();
        ...
        {
            LogHelper logger = LogHelper.NewLogger(typeof(Form_ClusterKmean), textBox_logPath.Text);
        ...
        private void process_OnExited(object sender, EventArgs e)
        {
            ...
            if (textBox_status.InvokeRequired)
            {
                textBox_status.Invoke(new MethodInvoker(delegate()
                {
                    LogHelper.GetLogger(typeof(Form_ClusterKmean)).Log(textBox_status.Text);
                }));
            }
        }
        ...
        public string Form_Execute(LogHelper logger, bool slient)
        {
            ...
            String command = 0 + " " + textBox_noctbd.Text + " " + textBox_stockPath.Text + " " + textBox_clusterPath.Text + " " + "NIL";
