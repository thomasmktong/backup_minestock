...
{
    public partial class Form_CatByAboveBelow : Form, Form_Automatable
    {
        Process process = null;
        Model_Automator automationModel = null;

        public Form_CatByAboveBelow()
        {
            InitializeComponent();
            ...
            textBox_genusPath.Text = Config.GENUS_PATH_DEFAULT;
        ...
        {
            FolderBrowserDialog comp_fbd = new FolderBrowserDialog();
            comp_fbd.SelectedPath = textBox_genusPath.Text;

            if (comp_fbd.ShowDialog() == DialogResult.OK)
            {
                textBox_genusPath.Text = comp_fbd.SelectedPath;
            }
        }
        ...
        {
            LogHelper logger = LogHelper.NewLogger(typeof(Form_CatByAboveBelow), textBox_logPath.Text);
        ...
        private void process_OnExited(object sender, EventArgs e)
        {
            ...
            if (textBox_status.InvokeRequired)
            {
                textBox_status.Invoke(new MethodInvoker(delegate()
                {
                    LogHelper.GetLogger(typeof(Form_CatByAboveBelow)).Log(textBox_status.Text);
                }));
            }
        }
        ...
        public void Form_ConfirmSettings(object sender, EventArgs e)
        {
            if (automationModel != null)
            {
                ...
                automationModel.Param3 = textBox_genusPath.Text;
                automationModel.Param4 = textBox_logPath.Text;
                automationModel.Param5 = textBox_refPeriod.Text;
            }
            this.Close();
        }
        ...
        {
            string toReturn = "";
            String file = Config.CATEGORIZE_STOCK_DATA_EXE;
            String command = 1 + " " + textBox_noctbd.Text + " " + textBox_refPeriod.Text + " " +
                textBox_stockPath.Text + " " + textBox_genusPath.Text + " " + "NIL";
